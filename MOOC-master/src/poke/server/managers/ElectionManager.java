...
public class ElectionManager implements ElectionListener {
	...
	public int getElectionCycle(){
		return this.electionCycle;
	}
	
	public void setLeaderId(Integer leader){
		this.leaderNode=leader;
	}
	
	public void setFirstTime(int first){
	...
	public void startElection() {
		//Added to reset votes when an election starts every time
		electionInstance().clear();
		electionCycle = electionInstance().createElectionID();
		System.out.println("****************Election Started by this election cycle" + electionCycle);
	...
	public void processRequest(Management mgmt) {
		...
		if (req.getAction().getNumber() == ElectAction.WHOISTHELEADER_VALUE) {
			respondToWhoIsTheLeader(mgmt);
			return;
		} else if (req.getAction().getNumber() == ElectAction.THELEADERIS_VALUE) {
			logger.info("********** Node " + conf.getNodeId() + " got an answer on who the leader is. Its Node "
					+ req.getCandidateId());
			System.out.println(req);
			System.out.println("********** Node CommitId "+Server.getCommitId()+" Leader's Commit Id "+req.getCommitId() );
			System.out.println("********** Node Election Cycle " +this.electionCycle +" Leader's Election Id "+req.getElectId());

			//Added to check if servers commit id is less then leaders commitid
			if(Server.getCommitId()<= req.getCommitId() && this.electionCycle < req.getElectId()) {

				System.out.println("Election Manager");
				this.electionCycle = req.getElectId();
				ElectionIDGenerator.setMasterID(electionCycle);
				this.leaderNode = req.getCandidateId();

				if((leaderNode!=null) && (conf.getNodeId()!= leaderNode)) {
...
/*	public void assessCurrentState(Management mgmt) {
	...
	public void respondToWhoIsTheLeader(Management mgmt) {
		...
		elb.setExpires(-1);
		System.out.println("$$$$$$$$$$$$$$$$$$$ Server Commit Id "+Server.getCommitId());
		elb.setCommitId(Server.getCommitId());//to latest transaction id
			...
			synchronized (syncPt) {
				...
				try {
					...
					if (election instanceof FloodMaxElection) {
						...
						System.out.println("Selected Raft as Election");
					}
