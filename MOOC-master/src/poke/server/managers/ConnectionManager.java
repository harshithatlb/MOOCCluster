...
public class ConnectionManager {
	...
	public static void addConnection(Integer nodeId, Channel channel, boolean isMgmt) {
		...
		if (isMgmt) {
			...
			System.out.println("******************** mgmt connections " + nodeId);
		}
		else
			connections.put(nodeId, channel);
		//System.out.println("******************** not a mgmt connections " + nodeId);
	...
	public synchronized static void removeConnection(Integer nodeId, boolean isMgmt) {
		if (isMgmt) {
			...
			System.out.println("*************removing in connection manager");
		}
		else
	...
	public synchronized static void broadcast(Management mgmt) {
		...
		for (Channel ch : mgmtConnections.values()) {
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^size of mgmt connection values  ^" + mgmtConnections.size());
			//System.out.println("^^^^^^^^^^^^^^^^^^^^^^^ " + mgmtConnections.values().hashCode());
			ch.write(mgmt);
		}
	}
