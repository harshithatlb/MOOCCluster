...
public class HeartbeatManager extends Thread {
	...
	ArrayList<Integer> notComingHB = new ArrayList<Integer>();
	...
	public Map<Channel, HeartbeatData> getOutgoingHB() {
		
		return outgoingHB;
	}

	public Map<Integer, HeartbeatData> getIncomingHB() {
		return incomingHB;
	}
	
	public ArrayList<Integer> getNotComingHB() {
		return notComingHB;
	}
	...
	public void processRequest(Management mgmt, Channel channel) {
		...
		if (hd == null) {
			...
			// logger.error("Unknown heartbeat received from node ",
			// mgmt.getHeader().getOriginator());
			return;
		} else {
			// if (logger.isDebugEnabled())
			System.out.println("************** HB received from " + mgmt.getHeader().getOriginator());
			// System.out.println("***********State of Outgoing HB map************");
			if (notComingHB.contains(mgmt.getHeader().getOriginator())){
				System.out.println("******************* Started Getting HB from "+mgmt.getHeader().getOriginator());
				//System.out.println("****************************"+notComingHB.toString());
				notComingHB.remove((Integer)mgmt.getHeader().getOriginator()) ;
				//System.out.println("@@@@@@@@@@@ Getting HB");
				if (ElectionManager.getInstance().leaderNode != null){
				if(ElectionManager.getInstance().leaderNode==conf.getNodeId())
				{
					ElectionManager.getInstance().respondToWhoIsTheLeader(mgmt);
					System.out.println("*************** Sending respondToWhoIsTheLeader to NodeID "+mgmt.getHeader().getOriginator());
				}
			}
			}
			/*for (Integer entry : notComingHB) {
				System.out.println("*******Not Getting HB from Node Id  " +entry);
				// entry.getKey().config() + " value " +
			...
			incomingHB.put(hd.getNodeId(), hd);
		}
	}
	 ...
	 * @param sa
	 */
	public void addOutgoingChannel(int nodeId, String host, int mgmtport,
			Channel ch, SocketAddress sa) {

		if (!outgoingHB.containsKey(ch)) {
			int port = incomingHB.get(nodeId).getPort();
			HeartbeatData heart = new HeartbeatData(nodeId, host, port,
					mgmtport);
			...
			try {
				...
				if (outgoingHB.size() > 0) {
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						try {
							if (logger.isDebugEnabled())
								logger.debug("sending heartbeat to "
										+ hd.getNodeId());
							...
							if (logger.isDebugEnabled())
								logger.debug("beat ("
										+ HeartbeatManager.conf.getNodeId()
										+ ") sent to " + hd.getNodeId()
										+ " at " + hd.getHost());
						} catch (Exception e) {
