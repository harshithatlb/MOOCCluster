		...
		if (req.getAction().getNumber() == NetworkAction.NODEJOIN_VALUE) {
			if (channel.isOpen()) {
				...
				if (socka != null) {
					...
					//if(HeartbeatManager.getInstance().getIncomingHB().containsKey(req.getFromNodeId())) {
						//Integer leaderNodeId = ElectionManager.getInstance().whoIsTheLeader();
						...
						System.out.println("*****************Adding node of existing cluster to outgoing HB");
					//} else{
						//System.out.println("********************The new node is not present in existing cluster");
					//}
				}
			} else
				logger.warn(req.getFromNodeId() + " not writable");
		} else if (req.getAction().getNumber() == NetworkAction.NODEDEAD_VALUE) {
			System.out.println("*******************Action received as Node_dead");

//			if(HeartbeatManager.getInstance().getOutgoingHB())
//			HeartbeatManager.getInstance().addOutgoingChannel(req.getFromNodeId(), isa.getHostName(),
//					isa.getPort(), channel, socka);
