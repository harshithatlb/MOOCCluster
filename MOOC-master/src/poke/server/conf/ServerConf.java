...
public class ServerConf {
	...
	private AdjacentCluster adjacentCluster;
	private List<ResourceConf> routing;

	private volatile HashMap<Integer, ResourceConf> idToRsc;
	//private int defaultNodeId = 1; //Added for raft
    private int elecTimeOut=100;//Added for Raft time in ms
...
/*	public int getDefaultNodeId(){
    ...
    public int getElecTimeOut(){
        return elecTimeOut;
    }

	public void setElecTimeOut(int elecTimeOut){
		this.elecTimeOut= elecTimeOut;
	}
	...
	public void addAdjacentClusterNode(NodeDesc node) {
		if (adjacentCluster == null)
			adjacentCluster = new AdjacentCluster();

		adjacentCluster.add(node);
	}
	...
	public AdjacentCluster getAdjacentCluster() {
		return adjacentCluster;
	}

	public void setAdjacentCluster(AdjacentCluster adjacentCluster) {
		this.adjacentCluster = adjacentCluster;
	}
	...
	public static final class AdjacentConf {
		...
		public NodeDesc getNode(Integer name) {
	...
	@XmlRootElement(name = "adjacentCluster")
	@XmlAccessorType(XmlAccessType.FIELD)
	public static final class AdjacentCluster {
		private TreeMap<Integer, NodeDesc> adjacentCluster;

		public NodeDesc getNode(Integer name) {
			return adjacentCluster.get(name);
		}

		public void add(NodeDesc node) {
			if (node == null)
				return;
			else if (adjacentCluster == null)
				adjacentCluster = new TreeMap<Integer, NodeDesc>();

			adjacentCluster.put(node.getNodeId(), node);
		}
