...
public class 	CommConnection {
	...
	public CommConnection(String host, int port, GeneratedMessage msg) {
		this.host = host;
		this.port = port;
	...
	private void init(GeneratedMessage msg) {
		// the queue to support client-side surging
		outbound = new LinkedBlockingDeque<com.google.protobuf.GeneratedMessage>();

		group = new NioEventLoopGroup();
		try {
			handler = new CommHandler(msg);
			...
			//channel.channel().pipeline().get(CommHandler.class);
	...
	private void init() {
		...
		try {
			...
			Bootstrap b = new Bootstrap();
			b.group(group).channel(NioSocketChannel.class).handler(new CommInitializer());
			b.option(ChannelOption.CONNECT_TIMEOUT_MILLIS, 10000);
			b.option(ChannelOption.TCP_NODELAY, true);
			b.option(ChannelOption.SO_KEEPALIVE, true);
	...
	protected class OutboundWorker extends Thread {
		...
		public void run() {
			...
			if (!forever) {
				CommConnection.logger.info("comm connection queue closing");
			}
		}
	}
